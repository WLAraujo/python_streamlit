...
# Criação da barra de filtros
st.sidebar.header("Filtros")

# Criação de uma caixa de seleção única para o ano
...
# Filtragem dos dados
df_times_pos = estatisticas_jogadores[(estatisticas_jogadores['Tm'].isin(times_selecionados)) &
                                      (estatisticas_jogadores['Pos'].isin(pos_sel))]

# Mudando os tipos de dados para numérico
lista_colunas = ['Age', 'G', 'GS', 'MP', 'FG', 'FGA', 'FG%', '3P', '3PA', '3P%', '2P', '2P%', 
                 'eFG%', 'FT', 'FTA', 'FT%', 'ORB', 'DRB', 'TRB', 'TRB', 'AST', 'STL', 'BLK', 
                 'TOV', 'PF', 'PTS']
for coluna in lista_colunas:
    df_times_pos[coluna] = pd.to_numeric(df_times_pos[coluna])
    
# Display das informações através de tabela
...
# Download dos dados e retorno de tabela como href
...
# Mostrando tabela através de markdown
